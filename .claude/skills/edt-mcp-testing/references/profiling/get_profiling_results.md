...
- For a NON-empty result you need a prior live profiling session: an active debug session (`launch` or `debug_yaxunit_tests`) → `start_profiling(applicationId=...)` → run code/tests → THEN `get_profiling_results`. Without that whole chain it returns `count: 0` (which is itself a valid, documented response and the normal state on a fresh workbench).
...
- **Pairs with `start_profiling`.** That tool TOGGLES profiling on the active debug target (calling it twice turns profiling back OFF). This tool only reads. To validate a real coverage report end-to-end: `debug_yaxunit_tests` (or `launch`) → `start_profiling(applicationId=...)` → run/finish the test → `get_profiling_results`. None of that is required just to exercise this tool's read path.
