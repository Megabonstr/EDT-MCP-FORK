...
Author a DCS default/variant settings tree through EDT's native model: total fields, selection, filters, sorting, nested groups, charts, conditional appearance and user-setting exposure. Supports surgical mutation with normalized before/after read-back. Parameters and examples: get_tool_guide('modify_dcs_settings').

## Parameters
| Parameter | Required | Type | Description |
| --- | --- | --- | --- |
| projectName | yes | string | EDT project name (required). |
| ownerFqn | yes | string | DCS owner FQN: Report, ExternalReport, or ExternalDataProcessor (required; external owners must already contain a DCS template). |
| settings | yes | object | Advanced settings specification (required). target='default' or target='variant' with variantName; omitted sections are preserved. See get_tool_guide for the full shape. |
